...
export default async function PartnerDashboardPage() {
  ...
  const applications = await db
    ...
    .where(eq(collegeCoachingApplications.partnerId, session.partnerId))
  ...
  const applicationsWithFileUrls = await Promise.all(
    applications.map(async (app) => {
      const appFiles = filesByApplication.get(app.id) ?? [];
      const withUrls = await Promise.all(
        appFiles.map(async (f) => ({
          ...f,
          url: await getSignedFileUrl(f.r2Key),
        }))
      );
    ...
    <div className="min-h-screen bg-yellow-50">
      ...
      <div className="container mx-auto px-4 sm:px-6 py-8">
        <h2 className="text-xl font-bold text-yellow-900 mb-4">
          Student Applications ({applicationsWithFileUrls.length})
        </h2>

        {applicationsWithFileUrls.length === 0 ? (
          <div className="bg-white rounded-lg shadow p-8 text-center text-gray-500">
            No applications yet.
          </div>
        ) : (
          <div className="flex flex-col gap-4">
            {applicationsWithFileUrls.map((app) => (
              <details
                key={app.id}
